...
# 1. rectification
# 2. glare
# 3. self.image_test distortion NO

# CONSTRAINTS
# 1. Puzzle is standard rectangular shape with 4 90-degree corners
# 2. end result will be provided to the robot (maybe) NO
# 3. all puzzle pieces will be face up with space in between

# Boundary Cases
# 1. same shape pieces with an self.image_test
# 2. different shape pieces with monotone color NO
# 3. different shape pieces with drastic color changes (each piece diff color) NO
# 4. same shape pieces with diff. color pieces (maybe)

# Algorithms
# 1. Edge Detection DONE
# 2. color information / filtering
# 3. puzzle solver matching adjacent pieces
# 4. stereo vision XXXXX
# 5. self.image_test segmentation / background subtracting / perspective compensation
# 6. feature matching? NO
...
def circularity(cnt):
    ...
    return (4 * np.pi * area) / (perimeter ** 2 + 1e-5)
...
def highestdivisor(num, edge_length):
    max = 0
...
def lowestdivisor(num, edge_length):
    min = num
    for k in reversed(range(num)[2:num]):
        if num % k == 0:
            # min = k
            height = (num / k) - 2
            width = k - 2
...
class PuzzleSolver:
    def __init__(self, location, flag=0):
        ...
        # print(self.Cap.num)
        # self.orig = cv2.imread(location + f"/piece{1}.png")
        # self.orig = cv2.imread(location + f"/piece{self.Cap.num}.png")'
        self.orig = cv2.imread(location + f"/wookie.png")
        #self.orig = self.orig[25:520,30:600]
        # self.orig = cv2.resize(self.orig, (self.orig.shape[1] << 1, self.orig.shape[0] << 1))
        self.binary, self.foreground = self.Seg.binarize(self.orig)
        # self.binary = cv2.imread(location + f"/binarypiece{self.Cap.num}.png")
        # self.binary = cv2.cvtColor(self.binary, cv2.COLOR_BGR2GRAY)
        # self.foreground = cv2.imread(location + f"/foregroundpiece{self.Cap.num}.png")
        ...
        if len(self.type["corner"]) == 4 and not self.type["square"] and not self.type["unknown"]:
            ...
            #        print("ITERATION", k)
            #        self.labeling()
            #        self.solution_matrix = self.solve(k, k+1)
            #    else:
            #        break
            if self.solution_matrix.size != 0:
    ...
    def solve(self, start=0, iteration=1):
        ...
        print(f"{total_list} START TOTAL LIST {len(total_list)}")
        # SOLVING ALGORITHM
        ref_side = 3
        puzzle_width = 0
        puzzle_height = 0
        ...
        while total_list:
                    ...
                    for indx, side in enumerate(self.piece[node]["sides"]):
                        ...
                        if t1 == -t2:  # potential pair if heads and hole
                            ...
                            if self.piece[node]["sides"][side_location][0] == 0:
                                ...
                                print(color_diff, node, closed_list[-1], "color")
                                print(overlap_diff, node, closed_list[-1], "area")
                    ...
                    if border_name in self.type["corner"] and len(corner_list) == len(self.type["corner"]) - 1:  # == 3
                        ...
                        if len(self.piece) % puzzle_width == 0:  # if integer/ multiple check
                            ...
                            if 2 * (side_width + side_height) == len(self.type["edge"]):  # perimeter check
                                ...
                                print(f"DIMENSIONS {puzzle_height} by {puzzle_width}: {completed_puzzle}")
                            else:
            ...
            # *******************MATCHING MIDDLES**************************
                ...
                for node in open_list:
                    ...
                    for indx, side in enumerate(self.piece[node]["sides"]):
                        ...
                        if t1 == -t2 and t3 == -t4 and t56 and t78:  # potential pair if heads and hole
                            ...
                            print(indx)
                            print(color_diff, node, last_matched, "color mid")
                            print(color_diff2, node, side_location, "color mid 2")
                            print(overlap_diff, node, last_matched, "area mid")
                            print(overlap_diff2, node, side_location, "area mid 2")
                ...
                print(f"{closed_list} MIDDLE CLOSED LIST")
                print(f"{open_list} MIDDLE OPEN LIST")
                print(f"{total_list} MIDDLE TOTAL LIST")
            else:
    ...
    def display(self):
        for c, i in enumerate(self.piece):
            ...
            for _, k in enumerate(self.piece[i]["corners"]):
                cv2.circle(self.labelimg, (int(k[0]), int(k[1])), 10, (0, 255, 128), -1)
        ...
        cv2.imshow("fore", self.foreimg_test)
        cv2.imshow("labelimg", self.labelimg)
        cv2.imshow("binary", self.binary_test)
        cv2.imshow("orig", self.orig)
        ...
        if self.piece[name]["sidecont"][matched_side].shape[0] and self.piece[name2]["sidecont"][ref_side].shape[0]:
            if self.piece[name]["sidecont"][matched_side].shape[0] >= self.piece[name2]["sidecont"][ref_side].shape[0]:
                ...
                for k, cont in enumerate(self.piece[name2]["sidecont"][ref_side]):
                    ...
                    # compare to ref_side
                    # store all comparisons in list
                    # print(f"{self.foreimg[cont[0][1],cont[0][0]]} {name2}")
                    # print(f"{self.foreimg[temp_list[k][0][1],temp_list[k][0][0]]} {name}")
                ...
                for k, cont in enumerate(self.piece[name]["sidecont"][matched_side]):
                    ...
                    c_score += m.sqrt((Lr - Lm) ** 2 + (ar - am) ** 2 + (br - bm) ** 2)
                    # E = m.sqrt((Lr - Lm) ** 2 + (ar - am) ** 2 + (br - bm) ** 2)
                    # C1 = m.sqrt(ar ** 2 + br ** 2)
                    # C2 = m.sqrt(am ** 2 + bm ** 2)
                    # c_score += deltaE(E, Lr - Lm, C1, C2)

        c_score = c_score / (len(temp_list) + 1e-5)
        # print(self.foreimg_test[self.piece[1]["sidecont"][0][0][0][1],self.piece[1]["sidecont"][0][0][0][0]])
        # cv2.circle(self.foreimg_test,(self.piece[1]["sidecont"][0][0][0][0],self.piece[1]["sidecont"][0][0][0][1]),5,(100,255,0),-1)
        # cv2.circle(self.foreimg_test, (200,100), 5,(100, 255, 0), -1)
        # cv2.drawContours(self.foreimg_test, self.piece[name]["sidecont"][matched_side], -1, (0, 255, 255), 3)
        # cv2.drawContours(self.foreimg_test, self.piece[name2]["sidecont"][ref_side], -1, (0, 255, 255), 3)
        # cv2.drawContours(self.foreimg_test, temp_list, -1, (0, 255, 255), 3)
    ...
    def getNeighbors(self, img, x, y, kernel):
        ...
        left = x - w
        right = (x + w) + 1
        section = img[top:bot, left:right]  # should be h,w,3 shape
        ...
        if section_shape == kernel_shape:
            ...
            # abc = []
            # calculate average of the remaining values
            for k in section:
    ...
    def headsnholes(self):
        ...
        for i in self.piece:
            ...
            #    s, e, f, d = defects[j, 0]
            #    start = tuple(self.piece[i]["cont"][s][0])
            #    end = tuple(self.piece[i]["cont"][e][0])
            #    if(d > 2000):
            #        point = tuple(self.piece[i]["cont"][f][0])
            #        cv2.circle(self.image_test, point, 5, [0, 0, 255], -1)
            #    cv2.line(self.image_test, start, end, [0, 255, 0], 2)

            # Heads/hole algorithm for piece i
            thresh = np.zeros(defects.shape[0])  # convexity pooints threshold
            for j in range(defects.shape[0]):
                _, _, _, d = defects[j, 0]
                thresh[j] = d
            ...
            # thresh = 3000  # convexity pooints threshold
            ...
            if len(point) > 0:
                ...
                for k in range(len(point)):
                    ...
                    for j in range(4):
                        ...
                        if i == 2 and k == 2 and j == 2:  # i is piece k is defect j is side
                            print(f"{area} area")
                            print(f"{seg_dis} distance")
                            print(f"{minA} minA")
                            print(f"{minD} minD")
                            cv2.drawContours(self.image_test, [a], -1, (255, 0, 0), 1)
                            cv2.line(self.image_test, self.piece[i]["cont"][point[k]][0],
                                     [int(segment[0]), int(segment[1])], (0, 0, 0), 3)
                        if j == 0:
                            b = point[k]
                            # minA = area
                            # minD = seg_dis
                        ...
                        if area + (area * 0.15) <= minA:  # if area significantly smaller
                            ...
                            # extra check
                            # cv2.line(self.image_test,self.piece[i]["cont"][point[k]][0],[int(segment[0]),int(segment[1])],(0,0,0),3)
                            if seg_dis <= minD:  # if
                                minD = seg_dis
                                minA = area
                                b = point[k]
                    ...
                    if len(loc[n]) == 2:
                        ...
                        for k in range(len(loc[n][0:-1])):  # USELESS FOR
                            ...
                            if head_area > box_area * tolerance:
                                # cntlist = np.concatenate((self.piece[i]["cont"][0:loc[n][k]],
                                #                          self.piece[i]["cont"][
                                #                          loc[n][k + 1]:len(self.piece[i]["cont"])]),
                                #                         axis=0)
                        ...
                        # line
                        # cntlist = np.array([ self.piece[i]["cont"][s][0], self.piece[i]["cont"][e][0] ])
                        # cv2.drawContours(self.image_test, [cntlist], -1, (0, 0, 0), 5)
                        if s <= e:
                        ...
                        if hole_area > box_area * tolerance:  # if the first cont is inbetween s and e
                            # cntlist = np.concatenate((self.piece[i]["cont"][0:s],
                            #                          self.piece[i][e:len(self.piece[i]["cont"])]),
                            #                         axis=0)
                        ...
                        for indx in range(int(len(cntlist) / 2)):
                            dis, _ = pnt2line(cntlist[len(cntlist) - 1 - indx][0], self.piece[i]["cont"][s][0],
                                              self.piece[i]["cont"][e][0])
                            # dis,_ = pnt2line(cntlist[len(cntlist)-1-indx][0],cntlist[0][0],cntlist[len(cntlist)-1][0])
                            if dis > thresh_d:
        ...
        # drawing = np.zeros((self.binary.shape[0], self.binary.shape[1], 1), dtype=np.uint8)
        # for i in self.piece:
        #    cv2.drawContours(drawing, self.piece[i]["cont"], -1, (255, 255, 255))
        #    cv2.drawContours(drawing, [self.piece[i]["hull"]], -1, 255)
        #
        # drawing = self.Seg.removeholes(drawing)
        # # Draw corners
        #
        # corner_mask = np.zeros((self.binary.shape[0], self.binary.shape[1], 1), dtype=np.uint8)
        # corner = cv2.cornerHarris(drawing, 10, 5, 0.05)
        # corner = cv2.dilate(corner, None, iterations=2)
        # thresh = 0.10 * corner.max()
        # corner_mask[corner > thresh] = [255]
        #
        # coordinate = np.argwhere(corner_mask)
        #
        # cv2.imshow("hull", drawing)
        # cv2.imshow("corner", corner_mask)
    ...
    def corners(self, i, cont, centroid, reorder = True):  # "corners": np.zeros((4, 2))
        ...
        # peaks = find_peaks_cwt(rho,widths=np.ones(rho.shape)*2 )
        ...
        # print(peaks[0])
        # print(np.degrees(theta[peaks[0]]))
        # print(rho[peaks[0]])
        # plt.figure()
        # if i == 8:
        #     plt.show()
    ...
    def reducepeaks(self, peaks, theta, rho, i):
        ...
        # rho = rho[peaks].tolist()
        # theta = theta[peaks].tolist()

        # print(peaks)
        # print(theta[peaks])
        # print(rho[peaks])

        # max = np.max(rho[peaks])
        # indx = np.where(rho == max)[0][0]
        # indx = np.where(peaks == indx)[0][0]
        # peaks = np.delete(peaks, indx)
        # print(indx)
        # print(peaks)
        # print(indx)
    ...
    def labeling(self):
        ...
        for i, cont in enumerate(contours):
            ...
            self.piece[i + 1] = {"cont": cont, "hull": hull, "centroid": center, "corners": box, "boxcorners": (x,y,w,h),
                                 "rotation": 0.,
                                 "sides": np.zeros((4, 3)),
                                 "sidecont": np.array([np.array([]), np.array([]), np.array([]), np.array([])],
                                                      dtype=object)}
            ...
            if not flag:
                ...
                self.cornerflag = 1
                for c in corner:
                    cv2.circle(self.labelimg,(c[0],c[1]),10,0,-1)
                cv2.imshow("errorimg", self.labelimg)
                cv2.waitKey(0)
    ...
    def show_solved(self):
        ...
        for r, rows in enumerate(self.solution_matrix):
            for c, i in enumerate(rows):
                ...
                img = cv2.warpPerspective(self.foreground, matrix, (width, height))

                # _, theta = cart2pol(self.piece[i]["corners"][2][0] - self.piece[i]["corners"][1][0],
                #                     self.piece[i]["corners"][2][1] - self.piece[i]["corners"][1][1])
                # if self.piece[i]["rotation"] >= 0:
                #     rotate_angle = np.degrees(self.piece[i]["rotation"] - theta)
                # else:
                #     rotate_angle = np.degrees(self.piece[i]["rotation"] + theta)
                # print(np.degrees(theta))
                # print(np.degrees(rotate_angle))
                #img = self.rotatesubimg(img, i)
                _, theta = cart2pol(self.piece[i]["corners"][2][0] - self.piece[i]["corners"][1][0],
                                    self.piece[i]["corners"][2][1] - self.piece[i]["corners"][1][1])
                theta = -theta
                theta = self.piece[i]["rotation"] - theta if self.piece[i]["rotation"]<0 else self.piece[i]["rotation"] + theta
        ...
        #     for c, i in enumerate(rows):
        #         # place head in hole
        #         for indx, s in enumerate(self.piece[i]["sides"]):
        #             if int(s[0]) == -1:
        #                 if indx == 0:  # top
        #                     piece_num = self.solution_matrix[r - 1, c]
        #                     solvedimg[r,c] = self.placeheadnhole(solvedimg[r,c], 2, self.piece[piece_num]["sidecont"][2])
        #                 elif indx == 1:  # left
        #                     piece_num = self.solution_matrix[r, c - 1]
        #                     solvedimg[r, c] = self.placeheadnhole(solvedimg[r,c], 3, self.piece[piece_num]["sidecont"][3])
        #                 elif indx == 2:  # bot
        #                     piece_num = self.solution_matrix[r + 1, c]
        #                     solvedimg[r, c] = self.placeheadnhole(solvedimg[r,c], 0, self.piece[piece_num]["sidecont"][0])
        #                 elif indx == 3:  # right
        #                     piece_num = self.solution_matrix[r, c + 1]
        #                     solvedimg[r, c] = self.placeheadnhole(solvedimg[r,c], 1, self.piece[piece_num]["sidecont"][1])

        return WOW

    def placeheadnhole(self,cflag=0):
        if cflag:
            return None,1
        ...
        for r, rows in enumerate(self.solution_matrix):
            for c, i in enumerate(rows):
                ...
                if flag:
                    ...
                    for c in subcorner:
                        cv2.circle(reduced_img,(c[0],c[1]),10,(255,0,255),-1)
                    cv2.imshow("sdf",reduced_img)
                    cv2.imshow("sdgfs",reduced_bin)
                    cv2.waitKey(0)
                    return None, 1
                else:
                    # for indx, k in enumerate(subcorner):
                    #     if indx == 0:
                    #         cv2.circle(reduced_img, (int(k[0]), int(k[1])), 4, (0, 255, 0), -1)
                    #     else:
                    #         cv2.circle(reduced_img, (int(k[0]), int(k[1])), 4, (0, 255, 255), -1)
                    ...
                    c_start += black_c_rgap - black_c_lgap#int(edistance(subcorner[0],subcorner[1]))
        ...
        #cv2.imshow("", crop_ffbin)

        return section, 0

    def rotatesubimg(self,img,i):
...
if __name__ == '__main__':
    ...
    img_original = cv2.imread("images/thepieces.png")
    ...
    cv2.imshow("labelimg", puzzle.labelimg)
    # print(pos.position(puzzle.piece, 5, 6, 1))
    # cv2.circle(puzzle.labelimg, (826, 332), 5, (0,255,255), -1)
    # cv2.circle(puzzle.labelimg, (243, 311), 5, (255, 0, 255), -1)
    # cv2.circle(puzzle.labelimg, (826, 320), 5, (255, 0, 255), -1)
    # cv2.circle(puzzle.labelimg, (243, 311), 5, (255, 0, 255), -1)
